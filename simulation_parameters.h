...
namespace parameters {
	...
	// output file
	const char *output_file_name = "25mev_xenon_y.root";
    ...
    //Turns off random event generation
    const bool uniform_events = true;

    ///-------------------------------------
    //--------WHAT events to generate?------
    ///-------------------------------------
    bool fixed_energy = true; //If gamma activity this will be set to true in the analyze_light.cpp
    double fixedE = 25.0; //Set to -1.0 to check if fixed value has been correctly assigned
	...
	const double y_position_range[2] {-150, 150};	// cm
	//const double z_position_range[2] {400,1000};	// cm [this comes from the "middle third"]
    const double z_position_range[2] {146,147.5};   // cm
